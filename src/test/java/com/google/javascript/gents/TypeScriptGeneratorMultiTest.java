...
public class TypeScriptGeneratorMultiTest {
    ...
    public void runTest() throws Exception {
        ...
        List<File> testFiles = getTestInputFilesRecursive(DeclarationGeneratorTest.JS, multiTestPath, this.dir.getName());
        ...
        GentsResult gentsResult = gents.generateTypeScript(sourceNames, sourceFiles, Collections.emptyList());
        ...
        File logFile = TypeScriptGeneratorTest.getTestDirPath(multiTestPath).resolve(this.dir.getName()).resolve("log.json").toFile();
        if (logFile.exists()) {
            String goldenLog = TypeScriptGeneratorTest.getFileText(logFile);
            // order independent comparison of the log entries.
            Set<LogItem> goldenLogItems = ImmutableSet.copyOf(new Gson().fromJson(goldenLog, LogItem[].class));
            Set<LogItem> emittedLogItems = ImmutableSet.copyOf(new Gson().fromJson(gentsResult.moduleRewriteLog, LogItem[].class));
    ...
    private static List<File> getTestInputFilesRecursive(FilenameFilter filter, String... dir) {
        ...
        for (File f : Files.fileTraverser().depthFirstPreOrder(TypeScriptGeneratorTest.getTestDirPath(dir).toFile())) {
