...
class ProgramSubject extends Subject {
     ...
     */
    private static final SourceFile CLUTZ_GOOG_BASE_TOTAL = SourceFile.fromFile(resource("src/test/java/com/google/javascript/clutz/testdata/base.js"), UTF_8);
     ...
     * passed in.
     */
    private static final SourceFile CLUTZ_GOOG_BASE_INCR = SourceFile.fromFile(resource("src/resources/partial_goog_base.js"), UTF_8);
    ...
    static ProgramSubject assertThatProgram(File singleInput) {
        return assertThatProgram(singletonList(singleInput), Collections.emptyList());
    }
    ...
    static final Subject.Factory<ProgramSubject, ProgramSubject.Program> FACTORY = new Subject.Factory<ProgramSubject, ProgramSubject.Program>() {
        @Override
        public ProgramSubject createSubject(FailureMetadata failureMetadata, ProgramSubject.Program that) {
    ...
    void generatesDeclarations(File golden) throws IOException {
        ...
        String stripped = DeclarationGeneratorTest.GOLDEN_FILE_COMMENTS_REGEXP.matcher(actual).replaceAll("");
        String expected = DeclarationGeneratorTest.getTestFileText(golden);
        String expectedClean = DeclarationGeneratorTest.GOLDEN_FILE_COMMENTS_REGEXP.matcher(expected).replaceAll("");
        if (!stripped.equals(expectedClean)) {
            ...
            if (System.getenv("UPDATE_GOLDENS") != null && !golden.getName().endsWith("_with_platform.d.ts") && expected.equals(expectedClean)) {
                Files.asCharSink(golden, Charsets.UTF_8).write(stripped);
            } else {
                check("generatedDeclarations()").withMessage("Verifying %s", golden.getName()).that(stripped).isEqualTo(expected);
            }
        }
    }
    ...
    private String[] parse() throws AssertionError {
        ...
        for (File nonroot : this.actual.nonroots) {
        ...
        for (File root : this.actual.roots) {
            sourceFiles.add(SourceFile.fromPath(root.toPath(), UTF_8));
            roots.add(root.getPath().replace(System.getProperty("file.separator"), "/"));
        }

        if (this.actual.sourceText != null) {
            sourceFiles.add(SourceFile.fromCode("main.js", this.actual.sourceText));
            roots.add("main.js");
        }
        ...
        if (this.withPlatform) {
            ...
            externFiles = Lists.newArrayList(SourceFile.fromFile(resource("src/resources/es6_min.js"), UTF_8));
        }

        if (this.depgraph != null) {
            opts.depgraph = DepgraphTest.parseFile(this.depgraph);
        } else {
            opts.depgraph = Depgraph.forRoots(roots, nonroots);
        }

        if (this.extraExternFile != null) {
            externFiles.add(SourceFile.fromFile(this.extraExternFile, UTF_8));
        }
        ...
        try {
            ...
            String dts = generator.generateDeclarations(sourceFiles, externFiles, Depgraph.forRoots(roots, nonroots));
