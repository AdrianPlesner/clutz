...
public class DeclarationSyntaxTest {
    private static final FilenameFilter JS_MULTIFILE_PARTIAL = (File dir, String name) -> JS_NO_EXTERNS.accept(dir, name) && dir.getName()
                                                                                                                                .equals("multifilePartial");

    private static final FilenameFilter JS_PARTIAL_CROSS_MODULE_TYPE_IMPORTS = (File dir, String name) -> JS_NO_EXTERNS.accept(dir, name) && dir.getName()
                                                                                                                                                .equals("partialCrossModuleTypeImports");

    public static final Path TSC = FileSystems.getDefault().getPath("node_modules", "typescript", "bin", "tsc");

    private static final ImmutableList<String> TSC_FLAGS = ImmutableList.of("--noEmit", "--skipDefaultLibCheck", "--downlevelIteration", "--lib",
                                                                            "es5,dom,es2015.iterable,esnext.asynciterable", "--noImplicitAny",
                                                                            "--strictNullChecks");
    ...
    public void testPartialCrossModuleTypeImportsDeclarationSyntax() throws Exception {
        List<File> inputs = DeclarationGeneratorTest.getTestInputFiles(JS_PARTIAL_CROSS_MODULE_TYPE_IMPORTS);
        doTestDeclarationSyntax(inputs);
    }

    @Test
    public void testAliasedInterfaceDeclarationSyntax() throws Exception {
        List<File> inputs = DeclarationGeneratorTest.getTestInputFiles(
                        (File dir, String name) -> TS_SOURCES.accept(dir, name) && dir.getName().equals("aliasedInterface"));
