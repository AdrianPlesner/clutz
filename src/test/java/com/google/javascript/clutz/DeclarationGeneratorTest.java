...
public class DeclarationGeneratorTest {
    ...
    private static FilenameFilter matchAllFilters(FilenameFilter filter, FilenameFilter... moreFilters) {
        return (dir, name) -> filter.accept(dir, name) && Arrays.stream(moreFilters).allMatch(f -> f.accept(dir, name));
    }

    private static FilenameFilter matchAnyFilter(FilenameFilter filter, FilenameFilter... moreFilters) {
        return (dir, name) -> filter.accept(dir, name) || Arrays.stream(moreFilters).anyMatch(f -> f.accept(dir, name));
    }
    ...
    private static ProgramSubject createProgramSubject(File input) {
        ...
        if (Arrays.asList("partial", "multifilePartial", "partialCrossModuleTypeImports").contains(input.getParentFile().getName())) {
    ...
    public static List<File> getTestInputFiles(FilenameFilter filter) {
        ...
        File[] testMultifilePartialFiles = getTestDataFolderPath().resolve("multifilePartial").toFile().listFiles(filter);
        ...
        File[] testPartialCrossModuleTypeImportsFiles = getTestDataFolderPath().resolve("partialCrossModuleTypeImports").toFile().listFiles(filter);
        // Output base files live in the 'outputBase' dir and impilicitly have base.js in their roots
        File[] testOutputBaseFiles = getTestDataFolderPath().resolve("outputBase").toFile().listFiles(filter);
        ...
        Stream<File> zipFiles = filesList.stream()
    ...
    public void runTest() throws Exception {
        File golden = getGoldenFile(this.input, ".d.ts");
        createProgramSubject(this.input).generatesDeclarations(golden);
    }
}
