...
public final class CollectModuleMetadata extends AbstractTopLevelCallback implements CompilerPass {
    ...
    void addFileMap(String filename) {
        if (!this.fileToModule.containsKey(filename)) {
            this.fileToModule.put(filename, new FileModule(filename, false));
        }
    }

    Map<String, FileModule> getNamespaceMap() {
        return this.namespaceToModule;
    }
    ...
    Map<String, FileModule> getSymbolMap() {
        ...
        for (FileModule module : this.fileToModule.values()) {
    ...
    public void visit(NodeTraversal t, Node n, Node parent) {
        ...
        @Nullable
        FileModule module = this.fileToModule.get(filename);
                ...
                switch (child.getFirstChild().getQualifiedName()) {
                    case "goog.module":
                        if (!parent.getFirstChild().equals(n)) { // is first statement
                            this.compiler.report(JSError.make(n, GentsErrorManager.GENTS_MODULE_PASS_ERROR, "goog.module must be the first top level statement."));
                            break;
                        }
    ...
    private void registerGoogModule(Node n, String file, String namespace) {
        if (this.fileToModule.containsKey(file)) {
            this.compiler.report(JSError.make(n, GentsErrorManager.GENTS_MODULE_PASS_ERROR, "goog.module files cannot contain other goog.module or goog.provides."));
            return;
        }
    ...
    void registerProvidesModule(Node n, String file, String namespace) {
        FileModule module;
        if (this.fileToModule.containsKey(file)) {
            if (this.fileToModule.get(file).isGoogModule) {
                this.compiler.report(JSError.make(n, GentsErrorManager.GENTS_MODULE_PASS_ERROR, "goog.provide cannot be used in the same file as goog.module."));
                return;
            }
            module = this.fileToModule.get(file);
        } else {
    ...
    class FileModule {
        ...
        private final Set<String> jsNamespaces = new HashSet<>();
        /**
        ...
        FileModule(String file, boolean isGoogModule) {
            ...
            this.isJsLibrary = !com.google.javascript.gents.CollectModuleMetadata.this.filesToConvert.contains(file);
        }
        ...
        boolean hasExports() {
            return !this.exportedNamespacesToSymbols.isEmpty();
        }
        ...
        void registerNamespaceToGlobalScope(String namespace) {
            this.providesObjectChildren.put(namespace, new LinkedHashSet<String>());
            if (this.isJsLibrary) {
                maybeAddExport(NodeUtil.newQName(com.google.javascript.gents.CollectModuleMetadata.this.compiler, namespace));
            }
            com.google.javascript.gents.CollectModuleMetadata.this.fileToModule.put(this.file, this);
            com.google.javascript.gents.CollectModuleMetadata.this.namespaceToModule.put(namespace, this);
        }
        ...
        private void maybeAddGoogExport(Node exportsName) {
            String fullname = this.providesObjectChildren.keySet().iterator().next();
            if (exportsName.matchesName("exports")) {
                String identifier = firstNonNull(exportsName.getNext().getQualifiedName(), com.google.javascript.gents.CollectModuleMetadata.this.nameUtil.lastStepOfName(fullname));
                ...
                if (!com.google.javascript.gents.CollectModuleMetadata.this.namespaceToModule.containsKey(importName)) {
                    com.google.javascript.gents.CollectModuleMetadata.this.namespaceToModule.put(importName, this);
                    this.providesObjectChildren.put(importName, ImmutableSet.of());
                }
            }
        }
        ...
        private void maybeAddProvidesExport(Node exportsName) {
            ...
            if (this.providesObjectChildren.containsKey(fullname)) {
                this.googProvideNamespaceToNode.put(fullname, exportsName);
                addExport(fullname, fullname, com.google.javascript.gents.CollectModuleMetadata.this.nameUtil.lastStepOfName(exportsName));

            } else if (exportsName.isGetProp() && this.providesObjectChildren.containsKey(exportsName.getFirstChild().getQualifiedName())) {
                this.googProvideNamespaceToNode.put(fullname, exportsName);
                ...
                @Nullable
                Node parentNode = this.googProvideNamespaceToNode.get(parentName);
                JSDocInfo jsDoc = parentNode != null ? NodeUtil.getBestJSDocInfo(parentNode) : null;

                if (this.providesObjectChildren.containsKey(parentName) && (jsDoc == null || !jsDoc.isConstructor())) {
                    addExport(fullname, fullname, com.google.javascript.gents.CollectModuleMetadata.this.nameUtil.lastStepOfName(exportsName));
                }
            }
        }
        ...
        private void addExport(String exportName, String importName, String identifier) {
            this.exportedNamespacesToSymbols.put(exportName, identifier);
            this.importedNamespacesToSymbols.put(importName, identifier);

            Node namespace = NodeUtil.newQName(com.google.javascript.gents.CollectModuleMetadata.this.compiler, importName);
            if (namespace.isGetProp()) {
                String parentName = namespace.getFirstChild().getQualifiedName();
                if (this.providesObjectChildren.containsKey(parentName)) {
                    this.providesObjectChildren.get(parentName).add(identifier);
                }
            }
        }
    }
