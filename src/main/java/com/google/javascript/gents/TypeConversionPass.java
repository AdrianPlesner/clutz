...
public final class TypeConversionPass implements CompilerPass {
    ...
    TypeConversionPass(AbstractCompiler compiler, CollectModuleMetadata modulePrepass, NodeComments nodeComments) {
    ...
    public void process(Node externs, Node root) {
        ...
        for (Node child : root.children()) {
            ...
            if (child.isScript()) {
                this.types = new LinkedHashMap<>();
                NodeTraversal.traverse(this.compiler, child, new TypeConverter());
                NodeTraversal.traverse(this.compiler, child, new TypeMemberConverter());
                NodeTraversal.traverse(this.compiler, child, new FieldOnThisConverter());
                NodeTraversal.traverse(this.compiler, child, new InheritanceConverter());
                NodeTraversal.traverse(this.compiler, child, new EnumConverter());
            }
        }
        convertTypeAlias();
    }
    ...
    private class TypeConverter extends AbstractPostOrderCallback {
            ...
            switch (n.getToken()) {
                ...
                case GETPROP -> {
                    ...
                    if (containsObject(typedefNode)) {
                        ...
                        com.google.javascript.gents.TypeConversionPass.this.typesToRename.put(n.getQualifiedName(), interfaceName);
                        com.google.javascript.gents.TypeConversionPass.this.typesToFilename.put(n.getQualifiedName(), n.getSourceFileName());
                        com.google.javascript.gents.TypeConversionPass.this.types.put(interfaceName, interfaceMember);
                ...
                case CLASS -> {
                    ...
                    if (jsDoc != null && jsDoc.isInterface()) {
                        ...
                        com.google.javascript.gents.TypeConversionPass.this.nodeComments.replaceWithComment(n, newNode);
                    }
                }
                default -> {
                }
        ...
        private void stripFunctionBody(Node member) {
            ...
            Node newFunction = new Node(Token.FUNCTION, functionName.detach(), functionParams.detach(), new Node(Token.EMPTY));
            newFunction.useSourceInfoFrom(functionNode);
            com.google.javascript.gents.TypeConversionPass.this.nodeComments.replaceWithComment(functionNode, newFunction);
        }
        ...
        private boolean containsObject(Node typedefNode) {
            ...
            if ((typedefToken == Token.QMARK || typedefToken == Token.BANG) && typedefNode.hasOneChild()) {
        ...
        private void createTypeAlias(Node n, Node parent) {
            ...
            if (bestJSDocInfo != null && bestJSDocInfo.hasTypedefType()) {
                ...
                com.google.javascript.gents.TypeConversionPass.this.nodeComments.moveComment(n, typeDef);
                com.google.javascript.gents.TypeConversionPass.this.types.put(name, typeDef);
    ...
    private class TypeMemberConverter extends AbstractPostOrderCallback {
        ...
        public void visit(NodeTraversal t, Node n, Node parent) {
            switch (n.getToken()) {
                ...
                case EXPR_RESULT -> {
                    ClassMemberDeclaration declaration = ClassMemberDeclaration.newDeclaration(n, com.google.javascript.gents.TypeConversionPass.this.types);
                    if (declaration == null) {
                        break;
                    }
    ...
    private class FieldOnThisConverter extends AbstractPostOrderCallback {
        ...
        public void visit(NodeTraversal t, Node n, Node parent) {
            if (n.isExprResult()) {
                ...
                if ("constructor".equals(fnName) && declaration.jsDoc != null && declaration.rhsEqualToField()) {
                    ...
                    for (Node param : params.children()) {
                        ...
                        @Nullable
                        JSTypeExpression paramType = constructorJsDoc == null ? null : constructorJsDoc.getParameterType(paramName);
                        ...
                        if (param.isDefaultValue() && Token.EQUALS.equals(paramType.getRoot().getToken())) {
                            paramType = new JSTypeExpression(paramType.getRoot().getFirstChild(), paramType.getSourceName());
                        }
                        // Names must be equal. Types must be equal, or if the declaration has no type it is
                        // assumed to be the type of the parameter.
                        if (declaration.memberName.equals(paramName) && (declarationType == null || declarationType.equals(paramType))) {

                            // Add visibility directly to param if possible
                            moveAccessModifier(declaration, param);
                            markAsConst(declaration, param);
                            com.google.javascript.gents.TypeConversionPass.this.compiler.reportChangeToEnclosingScope(n);
                            n.detach();
                            return;
                        }
                    }
        ...
        private void registerDeclaration(ClassMemberDeclaration declaration) {
            this.classFieldMap.put(declaration.classNode, declaration.memberName);
        }

        private boolean declarationHasBeenAdded(ClassMemberDeclaration declaration) {
            Collection<String> classMembers = this.classFieldMap.get(declaration.classNode);
    ...
    private class EnumConverter extends AbstractPostOrderCallback {
        ...
        public void visit(NodeTraversal t, Node n, Node parent) {
            switch (n.getToken()) {
                case VAR, LET, CONST -> {
                    ...
                    com.google.javascript.gents.TypeConversionPass.this.compiler.reportChangeToEnclosingScope(parent);
                }
                default -> {
                }
            }
        ...
        private Node transformMembers(Node members, boolean enumIsOfNumberType) {
            ...
            for (Node child : members.children()) {
                ...
                com.google.javascript.gents.TypeConversionPass.this.nodeComments.moveComment(child, newMember);
            }
            return enumMembers;
        }
    }
    ...
    private void convertTypeAlias() {
        Map<String, FileModule> fileMap = this.modulePrepass.getFileMap();
        for (Entry<String, String> entry : this.typesToRename.entrySet()) {
            ...
            String filename = this.typesToFilename.get(oldTypeName);
    ...
    private void convertConstructorToClass(Node n, JSDocInfo jsDoc) {
        ...
        if (jsDoc.getBaseType() != null) {
            ...
            String superClassName = jsDoc.getBaseType().getRoot().getFirstChild() // ignore the ! node as we always output non nullable types
                                         .getString();
            superClass = NodeUtil.newQName(this.compiler, superClassName);
        ...
        if (jsDoc.isInterface()) {
            ...
            Node constructor = IR.memberFunctionDef("constructor", IR.function(IR.name(""), params, body));
        ...
        this.nodeComments.replaceWithComment(n, typeNode);
    }
    ...
    private void convertDefineClassToClass(Node n) {
        ...
        this.nodeComments.replaceWithComment(n, classNode);
    }
    ...
    private boolean isConstructorInGoogDefineClass(Node n) {
        ...
        @Nullable
        Node stringKey = n.getParent();
        if (stringKey == null || !stringKey.isStringKey() || !"constructor".equals(stringKey.getString())) {
            return false;
        }

        @Nullable
        Node objectlit = stringKey.getParent();
        if (objectlit == null) {
            return false;
        }

        @Nullable
        Node call = objectlit.getParent();
        return call != null && call.getFirstChild().matchesQualifiedName("goog.defineClass");
    }
    ...
    private void convertObjectLiteral(Node classMembers, Node objectLiteralMember, boolean isStatic) {
        Preconditions.checkState(objectLiteralMember.isStringKey() || objectLiteralMember.isMemberFunctionDef());
        ...
        if (value.isFunction()) {
            ...
            this.nodeComments.moveComment(objectLiteralMember, n);
        } else {
    ...
    private void moveMethodsIntoClasses(ClassMemberDeclaration declaration) {
        ...
        if (declaration.classNode.getToken() == Token.INTERFACE) {
            ...
            if (body.hasChildren()) {
                this.compiler.report(JSError.make(declaration.rhs, GentsErrorManager.GENTS_CLASS_PASS_ERROR,
        ...
        this.nodeComments.moveComment(declaration.exprRoot, memberFunc);
        this.compiler.reportChangeToEnclosingScope(memberFunc);
    }
    ...
    private Node createMemberVariableDef(ClassMemberDeclaration declaration) {
        ...
        this.nodeComments.moveComment(declaration.exprRoot, fieldNode);
        return fieldNode;
    }
    ...
    private void moveFieldsIntoClasses(ClassMemberDeclaration declaration) {
        ...
        if (declaration.rhs == null) {
            ...
            this.nodeComments.clearComment(declaration.exprRoot);
        }
    ...
    private boolean canPromoteFieldInitializer(ClassMemberDeclaration declaration) {
        ...
        if (fnNode != null) {
            String fnName = getEnclosingFunctionName(fnNode);
            return "constructor".equals(fnName);
        }
    ...
    private void maybeRemoveInherits(Node exprNode) {
        ...
        if (exprNode.getFirstChild().isCall()) {
            ...
            if (!this.types.containsKey(className)) {
                this.compiler.report(JSError.make(exprNode, GentsErrorManager.GENTS_CLASS_PASS_ERROR, String.format("Class %s could not be found.", className)));
                return;
            }

            // Check that superclass is consistent
            Node classNode = this.types.get(className);
            ...
            if (classNode.getSecondChild().isEmpty() || !storedSuperClassName.equals(superClassName)) {
                this.compiler.report(JSError.make(exprNode, GentsErrorManager.GENTS_CLASS_PASS_ERROR, String.format("Invalid superclass for %s", className)));
                return;
            }

            this.compiler.reportChangeToEnclosingScope(exprNode);
            exprNode.detach();
            ...
            if (lhs.isGetProp() && "prototype".equals(lhs.getLastChild().getString())) {
                this.compiler.report(JSError.make(exprNode, GentsErrorManager.GENTS_CLASS_PASS_ERROR,
                                                  String.format("Cannot directly assign to prototype for %s", lhs.getFirstChild().getQualifiedName())));
            }
        }
    }
    ...
    private void maybeReplaceSuperCall(Node callNode) {
        ...
        if (callName.equals(className + ".base") && callNode.getSecondChild().isThis()) {
            ...
            if (classNode.getSecondChild().isEmpty()) {
                this.compiler.report(JSError.make(callNode, GentsErrorManager.GENTS_CLASS_PASS_ERROR,
            ...
            if ("constructor".equals(methodName)) {
                this.nodeComments.replaceWithComment(callNode.getFirstChild(), IR.superNode());
            } else {
                this.nodeComments.replaceWithComment(callNode.getFirstChild(), NodeUtil.newQName(this.compiler, "super." + methodName));
            }
        ...
        if (callName.startsWith(superClassName + ".prototype.") && callName.endsWith(".call")) {
            if (callNode.getSecondChild().isThis()) {
                ...
                this.nodeComments.replaceWithComment(n, IR.superNode());
                this.nodeComments.replaceWithComment(callNode.getFirstChild(), nameNode);
                callNode.removeChild(callNode.getSecondChild());
                this.compiler.reportChangeToEnclosingScope(callNode);
            }
        }
    ...
    private void addTypeToScope(Node n, String typeName) {
        if (this.types.containsKey(typeName)) {
            this.compiler.report(JSError.make(n, GentsErrorManager.GENTS_CLASS_PASS_ERROR, String.format("Type %s has been defined multiple times.", typeName)));
            return;
        }
        this.types.put(typeName, n);
    }
    ...
    private static class ClassMemberDeclaration {
        ...
        boolean rhsEqualToField() {
            return this.rhs != null && this.rhs.matchesQualifiedName(this.memberName);
        }
        ...
        static ClassMemberDeclaration newDeclaration(Node n, Map<String, Node> classes) {
            ...
            String className = isStatic ? fullName.getFirstChild().getQualifiedName() : fullName.getFirstFirstChild().getQualifiedName();
        ...
        static boolean isStatic(Node fullName) {
            return !(fullName.getFirstChild().isGetProp() && "prototype".equals(fullName.getFirstChild().getLastChild().getString()));
        }
    }
}
