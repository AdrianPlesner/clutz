...
public final class RemoveGoogScopePass extends AbstractTopLevelCallback implements CompilerPass {
    ...
    public void visit(NodeTraversal t, Node n, Node parent) {
        ...
        switch (callName) {
            case "goog.provide", "goog.require" -> {
                ...
                this.providedNamespaces.add(maybeCallNode.getLastChild().getString());
            }
            case "goog.scope" -> {
                rewriteGoogScope(n);
            }
    ...
    private void rewriteGoogScope(Node n) {
        ...
        @Nullable
        Node nodeToMove = blockOfScopeContents.getFirstChild();
    ...
    private RewriteStatus maybeRecordAndRemoveAlias(Node assign) {
        ...
        if (isInProvidedNamespace(rhs)) {
            this.aliasToProvidedNamespace.put(lhs.getString(), rhs.getQualifiedName());
    ...
    private boolean isInProvidedNamespace(Node node) {
        for (String providedNamespace : this.providedNamespaces) {
            @Nullable
            String rhsQualifiedName = node.getQualifiedName();
    ...
    private void maybeReassignAlias(Node assign) {
        ...
        if (this.aliasToProvidedNamespace.containsKey(alias)) {
            String providedNamespace = this.aliasToProvidedNamespace.get(alias);

            String suffix = lhs.getQualifiedName().substring(alias.length());
            Node fullName = NodeUtil.newQName(this.compiler, providedNamespace + suffix);
