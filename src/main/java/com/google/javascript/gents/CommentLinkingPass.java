...
public final class CommentLinkingPass implements CompilerPass {
    ...
    private static final Pattern EMPTY_COMMENT_REGEX = Pattern.compile("^\\s*(\\/\\/|\\/\\*(\\s|\\*)*\\*\\/)\\s*$");
    ...
    private static final Pattern[] JSDOC_REPLACEMENTS_NO_KEEP = {
                    ...
                    Pattern.compile(BEGIN_JSDOC_LINE + "@(private|protected|public|package|const|enum)[ \t]*(\\{.*\\})?[ \t]*" + EOL),
     ...
     */
    private static final ImmutableSet<Token> TOKENS_CAN_HAVE_EMPTY_CHILD = ImmutableSet.of(Token.SCRIPT, Token.MODULE_BODY, Token.BLOCK);
    /**
    ...
    private static final ImmutableSet<Token> TOKENS_IGNORE_COMMENTS = ImmutableSet.of(Token.LABEL_NAME, Token.GENERIC_TYPE, Token.BLOCK);
    ...
    public void process(Node externs, Node root) {
        for (Node script : root.children()) {
            if (script.isScript()) {
                ...
                // immutable list.
                ImmutableList<Comment> comments = ImmutableList.copyOf(this.compiler.getComments(script.getSourceFileName()));
                NodeTraversal.traverse(this.compiler, script, new LinkCommentsForOneFile(comments));
            }
        }
    }
    ...
    private class LinkCommentsForOneFile implements NodeTraversal.Callback {
        ...
        private final List<Comment> commentBuffer = new ArrayList<>();
        ...
        private void addNextCommentToBuffer() {
            if (hasRemainingComments()) {
                this.commentBuffer.add(getNextComment());
            }
            this.lastCommentIndex++;
        }
        ...
        private void linkCommentBufferToNode(Node n) {
            ...
            String sep = "\n";
            for (Comment c : this.commentBuffer) {
            ...
            if (!comment.isEmpty()) {
                com.google.javascript.gents.CommentLinkingPass.this.nodeComments.addComment(n, comment);
            }
        ...
        private boolean canHaveComment(Node n) {
            ...
            Node parent = n.getParent();
            return parent == null || !parent.isGetProp() || parent.getSecondChild() != n;
        }
        ...
        private boolean isCommentAdjacentToLine(int line) {
            ...
            return commentLine == line || (commentLine == line - 1 && commentLine != getFirstLineOfNextComment());
        }
        ...
        private boolean linkFunctionArgs(Node n, int line) {
            if (n.getNext() == null) {
                ...
                if (endOfComment < startOfNextNode) {
                    ...
                    String lineContents = com.google.javascript.gents.CommentLinkingPass.this.compiler.getInput(new InputId(n.getSourceFileName())).getSourceFile().getLine(line);
