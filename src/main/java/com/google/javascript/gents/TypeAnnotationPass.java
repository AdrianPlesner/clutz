...
public final class TypeAnnotationPass implements CompilerPass {
    ...
    TypeAnnotationPass(AbstractCompiler compiler, PathUtil pathUtil, NameUtil nameUtil, Map<String, FileModule> symbolMap,
                       Table<String, String, String> typeRewrite, NodeComments nodeComments, Map<String, String> externsMap) {
    ...
    public void process(Node externs, Node root) {
        NodeTraversal.traverse(this.compiler, root, new TypeAnnotationConverter());
        NodeTraversal.traverse(this.compiler, root, new AccessModifierConverter());
    ...
    private class TypeAnnotationConverter extends AbstractPostOrderCallback {
        ...
        private void maybeSetInlineTypeExpression(Node nameNode, Node commentNode, JSDocInfo docInfo, boolean isReturnType) {
            if (docInfo == null) {
                return;
            }
            ...
            String nodeComment = com.google.javascript.gents.TypeAnnotationPass.this.nodeComments.getComment(commentNode);
            if (nodeComment == null) {
                return;
            }
            nodeComment = nodeComment.replaceFirst("\\n?" + Pattern.quote(toRemove), "");
            com.google.javascript.gents.TypeAnnotationPass.this.nodeComments.setComment(commentNode, nodeComment);
            com.google.javascript.gents.TypeAnnotationPass.this.compiler.reportChangeToEnclosingScope(commentNode);
        }
        ...
        private boolean setParameterTypeFromFunctionDoc(Node node, Node parent) {
            ...
            if (parameterType.getRoot().getToken() == Token.ITER_REST) {
                ...
                com.google.javascript.gents.TypeAnnotationPass.this.nodeComments.replaceWithComment(node, attachTypeExpr);
            }
            ...
            if (parameterType.getRoot().getToken() == Token.EQUALS) {
                ...
                } else if (node.getParent().getChildAtIndex(1).isName() && node.getParent().getChildAtIndex(1).getString().equals("undefined")) {
    ...
    void addTypeOnlyImports(Node script) {
        if (!script.isScript() || !this.importsNeeded.containsKey(script.getSourceFileName())) {
            return;
        }
        ...
        for (Node n : body.children()) {
            if (n.isImport()) {
                ...
                for (Node newImport : this.importsNeeded.get(script.getSourceFileName())) {
    ...
    private TypeDeclarationNode indexSignatureType(TypeDeclarationNode keyType, TypeDeclarationNode valueType) {
    ...
    private String convertTypeName(String sourceFile, String typeName) {
        Map<String, String> rewriteMap = this.typeRewrite.containsRow(sourceFile) ? this.typeRewrite.rowMap().get(sourceFile) : new HashMap<>();

        // All type symbols declared anywhere in the compilation unit
        Set<String> allTypes = new HashSet<>();
        allTypes.addAll(rewriteMap.keySet());
        allTypes.addAll(this.symbolToModule.keySet());

        String importedNamespace = this.nameUtil.findLongestNamePrefix(typeName, allTypes);
        ...
        if (rewriteMap.containsKey(importedNamespace)) {
            ...
            return this.nameUtil.replacePrefixInName(typeName, importedNamespace, symbol);
        } else {
            // Rewrite ALL types in compilation unit
            FileModule module = this.symbolToModule.get(importedNamespace);
            String symbol = module.importedNamespacesToSymbols.get(importedNamespace);

            FileModule typeModule = this.symbolToModule.get(typeName);
            ...
            if (typeModule != null && !sourceFile.equals(typeModule.file)) {
                ...
                if (module.shouldUseOldSyntax()) {
                    ...
                    importFile = Node.newString(this.pathUtil.getImportPath(sourceFile, module.file));
                }
                Node importNode = new Node(Token.IMPORT, IR.empty(), importSpec, importFile);
                this.importsNeeded.put(sourceFile, importNode);
            }
            this.typeRewrite.put(sourceFile, importedNamespace, symbol);
            return this.nameUtil.replacePrefixInName(typeName, importedNamespace, symbol);
        }
    }
    ...
    private void flatten(Iterable<TypeDeclarationNode> types, List<TypeDeclarationNode> result, boolean hasNull) {
