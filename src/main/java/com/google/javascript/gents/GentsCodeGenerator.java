...
public class GentsCodeGenerator extends CodeGenerator {
    ...
    GentsCodeGenerator(CodeConsumer consumer, CompilerOptions options, NodeComments nodeComments, Map<String, String> externsMap) {
    ...
    protected void add(Node n, Context ctx) {
        ...
        String comment = this.nodeComments.getComment(n);
        if (comment != null) {
    ...
    private static final ImmutableSet<Token> TOKENS_TO_ADD_NEWLINES_BEFORE = ImmutableSet.of(Token.CLASS, Token.EXPORT, Token.FUNCTION, Token.INTERFACE,
                                                                                             Token.MEMBER_FUNCTION_DEF);

    /** Add newlines to the generated source. */
    private void maybeAddNewline(Node n) {
        boolean hasComment = this.nodeComments.hasComment(n) || this.nodeComments.hasComment(n.getParent()) || isPreviousEmptyAndHasComment(n) || (n.getParent() != null
                        && isPreviousEmptyAndHasComment(n.getParent()));
    ...
    private boolean isPreviousEmptyAndHasComment(Node n) {
        ...
        return prev != null && prev.isEmpty() && this.nodeComments.hasComment(prev);
    }
    ...
    private boolean maybeOverrideCodeGen(Node n) {
        ...
        switch (n.getToken()) {
            ...
            case ANY_TYPE -> {
                ...
                String anyTypeName = this.externsMap.get("any");
                if (anyTypeName != null) {
                    add(anyTypeName);
                    return true;
                }
