...
class DeclarationGenerator {
     ...
     * </pre>
     */
    private static final ImmutableSet<String> RESERVED_JS_WORDS = ImmutableSet.of("break", "case", "catch", "class", "let", "continue", "debugger", "default",
     ...
     */
    private static final ImmutableMap<String, String> KNOWN_CLASS_ALIASES = ImmutableMap.of("goog.log.Logger", "goog.debug.Logger", "goog.log.Level",
                                                                                            "goog.debug.Logger.Level", "goog.log.LogRecord",
    ...
    static final DiagnosticType CLUTZ_MISSING_TYPES = DiagnosticType.error("CLUTZ_MISSING_TYPES",
                                                                           "A dependency does not compile because it is missing some types. This is often "
                                                                           + "caused by"
                                                                                           + " the referenced code missing dependencies or by missing externs"
                                                                                           + " in your build"
                                                                                           + " rule.");
     ...
     */
    @Nullable
    private JSType iterableType;

    @Nullable
    private JSType iteratorIterableType;
    ...
    DeclarationGenerator(Options opts) {
        ...
        this.errorManager = new ClutzErrorManager(System.err, ErrorFormat.MULTILINE.toFormatter(this.compiler, true), opts.debug);
    ...
    void collectTypedefs() {
        for (TypedVar var : this.compiler.getTopScope().getAllSymbols()) {
            ...
            if (type == null || !isTypedef(type) || var.getName().startsWith("window.") || isPrivate(var.getJSDocInfo())) {
                continue;
            }

            JSType realType = this.compiler.getTypeRegistry().getGlobalType(var.getName());
            if (realType != null && shouldEmitTypedefByName(realType) && !this.typedefs.containsKey(realType) && !PlatformSymbols.TYPESCRIPT_LIB_D_TS.contains(
                            var.getName()) && !PlatformSymbols.CLOSURE_EXTERNS_NOT_USED_IN_TYPESCRIPT.contains(var.getName())) {
    ...
    void generateDeclarations() {
        ...
        for (String source : this.opts.arguments) {
            ...
            getJsEntryPathsFromZip(source).stream().map((Path p) -> SourceFile.fromPath(p, UTF_8)).forEach(sourceFiles::add);
        }
    ...
    String generateDeclarations(List<SourceFile> sourceFiles, List<SourceFile> externs, Depgraph depgraph) throws AssertionError {
        ...
        if (this.opts.partialInput) {
            this.importRenameMap = new ImportRenameMapBuilder().build(this.compiler.getParsedInputs(), this.opts.depgraph.getGoogProvides());
            this.aliasMap = new AliasMapBuilder().build(this.compiler.getParsedInputs(), this.opts.depgraph.getGoogProvides());
            this.legacyNamespaceReexportMap = new LegacyNamespaceReexportMapBuilder().build(this.compiler.getParsedInputs(),
                                                                                            this.opts.depgraph.getGoogProvides());
    ...
    private boolean shouldSkipVar(TypedVar var) {
        return this.opts.skipEmitPattern != null && this.opts.skipEmitPattern.matcher(var.getInputName()).matches();
    }
    ...
    private void declareTypedefNamespace(TypedVar typedef, JSType typedefType, Set<String> provides) {
        ...
        new TreeWalker(this.compiler.getTypeRegistry(), provides, false, false).visitTypeAlias(typedefType, typedefName, false);
    ...
    private void processReservedSymbols(NavigableSet<String> provides, TypedScope topScope) {
        ...
        while (it.hasNext()) {
          if (collapsedNamespaces.contains(getNamespace(it.next()))) {
            it.remove();
          }
        }
    }
     ...
     */
        ...
        while (maxTypeUsedDepth > 0) {
            ...
            for (TypedVar symbol : this.compiler.getTopScope().getAllSymbols()) {
                ...
                if (!this.typesUsed.contains(name) || typesEmitted.contains(name) || (!transitiveProvides.contains(name) && typesEmitted.contains(namespace))) {
                    continue;
                }

                // skip provided symbols (as default or in an namespace).
                if (provides.contains(name) || (!transitiveProvides.contains(name) && provides.contains(namespace))) {
                    continue;
                }
              ...
              if (symbolInput == null || symbolInput.isExtern()) {
                ...
                declareNamespace(namespace, symbol, name,
                                /* isDefault */ true, Collections.emptySet(),
                                /* isExtern */ false);
                typesEmitted.add(name);
            }
            // if no new types seen, safely break out.
          if (this.typesUsed.size() == typesUsedCount) {
            break;
          }
            maxTypeUsedDepth--;
        }
     ...
     *
     * @param provides
     *                 A collection of goog.provide symbols
     * @param name
    ...
    private void declareLegacyNamespaceAliases() {
        if (!this.legacyNamespaceReexportMap.isEmpty()) {
            for (Entry<String, String> e : this.legacyNamespaceReexportMap.entrySet()) {
                ...
                TreeWalker treeWalker = new TreeWalker(this.compiler.getTypeRegistry(), new LinkedHashSet<>(), false, false);
    ...
    private void processExternSymbols() {
        ...
        for (TypedVar symbol : externSymbols) {
          ...
          if (!isDefiningType(symbol.getType()) && visitedClassLikes.contains(parentPath)) {
            ...
            declareNamespace(isDefault ? parentPath : symbol.getName(), symbol, emitName, isDefault, shadowedSymbols, true);
    ...
    private static final Ordering<TypedVar> BY_SOURCE_FILE = Ordering.natural()
                                                                     .onResultOf((@org.checkerframework.checker.nullness.qual.Nullable TypedVar input) -> {
                                                                       if (input == null) {
                                                                         return null;
                                                                       }
    ...
    private static final Ordering<TypedVar> BY_VAR_NAME = Ordering.natural()
                                                                  .onResultOf((@org.checkerframework.checker.nullness.qual.Nullable TypedVar input) -> {
                                                                    if (input == null) {
                                                                      return null;
                                                                    }
    ...
    private static final Ordering<TypedVar> BY_SOURCE_FILE_AND_VAR_NAME = BY_SOURCE_FILE.compound(BY_VAR_NAME);
    ...
    private boolean isDefaultExport(TypedVar symbol) {
      ...
      if (otype != null && otype.getOwnPropertyNames().isEmpty()) {
        ...
        return !symbol.getType().isObject() || symbol.getType().isInterface() || symbol.getType().isInstanceType() || symbol.getType().isEnumType()
    ...
    private void declareNamespace(String namespace, TypedVar symbol, String emitName, boolean isDefault, Set<String> provides, boolean isExtern) {
        ...
        boolean isGoogNamespace = GOOG_BASE_NAMESPACE.equals(namespace) && GOOG_BASE_NAMESPACE.equals(symbol.getName());
        if (isGoogNamespace) {
        ...
        TreeWalker treeWalker = new TreeWalker(this.compiler.getTypeRegistry(), provides, isExtern, isGoogNamespace);
        ...
        if (isDefault) {
            ...
            Set<String> propertyNames = objType != null ? objType.getOwnPropertyNames() : Collections.emptySet();
            ...
            for (TypedVar propertySymbol : allSymbols) {
                ...
                if (desiredSymbols.contains(propertyName) && propertySymbol.getType() != null && !propertySymbol.getType().isFunctionPrototypeType()
                                && !isPrototypeMethod(propertySymbol)) {
                    if (!isValidJSProperty(getUnqualifiedName(propertySymbol))) {
                        emit("// skipping property '" + getUnqualifiedName(propertyName) + "' because it is not a valid symbol.");
                        emitBreak();
                        continue;
                    }
                    ...
                    if (propertyName.equals("goog.require")) {
                        emit("function require (name : string ) : " + Constants.INTERNAL_NAMESPACE + ".ClosureSymbolNotGoogProvided;");
            ...
            if (oType != null) {
                ...
                for (String desiredSymbol : desiredSymbols) {
                    ...
                    treeWalker.visitProperty(propName, oType, isStatic, forcePropDeclaration, isNamespace, Collections.emptyList());
                }
            }
        }
        emitNamespaceEnd();
     ...
     */
    private void maybeQueueForInnerWalk(boolean isExtern, Map<String, ObjectType> symbolsToInnerWalk, TypedVar propertySymbol, String propertyName) {
    ...
    private boolean isDefiningType(JSType propType) {
        return isClassLike(propType) || propType.isEnumType() || propType.isInterface() || isTypedef(propType);
    }
    ...
    private boolean isClassLike(JSType propType) {
        ...
        return !isTypedef(propType) && (propType.isConstructor() || propType.isInterface())
                        ...
                        && !propType.toMaybeObjectType().isNativeObjectType() && !propType.isFunctionPrototypeType();
    }
    ...
    private boolean isPrivate(JSType type) {
        ...
        if (this.typedefs.containsKey(type) || (nType != null && this.typedefs.containsKey(nType.getReferencedType()))) {
            return false;
        }
    ...
    private boolean isPrivate(@Nullable JSDocInfo docInfo) {
        ...
        return docInfo.getVisibility() == Visibility.PRIVATE || docInfo.getVisibility() == Visibility.PACKAGE;
    }
    ...
    private void declareModule(String name, boolean isDefault, String emitName, boolean inParentNamespace) {
    ...
    static List<SourceFile> getDefaultExterns(Options opts) {
        try {
            return AbstractCommandLineRunner.getBuiltinExterns(opts.getCompilerOptions().getEnvironment());
        } catch (IOException e) {
     ...
     *
     * @param file
     *                 Original .js file that contained JS symbol for which module/namespace is being
     *                 generated.
     */
    ...
    private class TreeWalker {
        ...
        private TreeWalker(JSTypeRegistry typeRegistry, Set<String> provides, boolean isExtern, boolean isGoogNamespace) {
            ...
            if (type.isFunctionType() && !isNewableFunctionType(type.toMaybeFunctionType())) {
                ...
                if (!isAliasedClassOrInterface(symbol, ftype)) {
                    ...
                    if (KNOWN_CLASS_ALIASES.containsKey(symbol.getName())) {
                        visitKnownTypeValueAlias(getUnqualifiedName(symbol), KNOWN_CLASS_ALIASES.get(symbol.getName()));
                    } else {
                ...
                if (isTypedef(type)) {
                    ...
                    if (registryType != null) {
                        ...
                        emitComment("Intended to visit type alias '" + symbol.getName() + " but type not found in Closure type registry.");
                    }
                }
         ...
         * @param alreadyEmittedTemplateType
         *                 when visiting methods the class template types will be
         *                 reported by closure, but should not be emitted.
         * @param isDeclaration
         *                 isDeclaration {@code true} if this type declares the template types,
         *                 {@code false} if it instantiates a generic type. In the former case, Clutz emits defaults
         *                 for the template parameters.
         */
        private void visitTemplateTypes(ObjectType type, List<String> alreadyEmittedTemplateType, boolean isDeclaration) {
        ...
        private void visitTypeAlias(JSType registryType, String unqualifiedName, boolean emitNeverBrand) {
        ...
        private void visitTypeAliasForMixedStringEnums(JSType registryType, String unqualifiedName, Set<String> literalTypes) {
            emitTypeAliasPrefix(registryType, unqualifiedName);
            emit("&{clutzEnumBrand: never}");
         ...
         */
        private boolean maybeStringOrNumericEnum(EnumType enumType, String unqualifiedName, Node objectOfAllMembers, Map<String, Node> elements) {
        ...
        private void visitTypeAsPrimary(JSType type) {
            ...
            if (!isLiteralFunction(type) && (type.isConstructor() || type.isFunctionType() || type.isUnionType())) {
                emit("(");
                visitType(type);
                emit(")");
            } else {
        ...
        private Void emitTemplatizedType(TemplatizedType type, boolean inImplementsExtendsPosition) {
            ...
            if (this.typeRegistry.getNativeType(ARRAY_TYPE).equals(referencedType) && type.getTemplateTypes().size() == 1) {
         ...
         *
         * @param isStatic
         *                 if true, skips properties that are likely namespaces during extern
         *                 processing.
         */
        private Set<String> getTypePropertyNamesToEmit(final ObjectType type, boolean isStatic) {
            return Sets.filter(getEmittablePropertyNames(type), propName -> {
        ...
        private Set<String> getEmittablePropertyNames(final ObjectType type) {
            return sorted(Sets.filter(type.getOwnPropertyNames(),
                                      propName -> isEmittableProperty(type, propName) && !isTypeCheckSuppressedProperty(type, propName)));
        }
        ...
        private void visitObjectType(FunctionType type, ObjectType prototype, List<String> classTemplateTypeNames) {
            emit("{");
            indent();
            emitBreak();
            ...
            checkArgument(instanceType.isObject(),
                          "expected an ObjectType for this, but got " + instanceType + " which is a " + instanceType.getClass().getSimpleName());
            ...
            // original intent.
            visitInstanceProperties((ObjectType) instanceType, Collections.emptySet(), Collections.emptySet(), Collections.emptyList());

            // Bracket-style property access for dictionnary...
            if (type.isDict()) {
                emitIndexSignature(DeclarationGenerator.this.compiler.getTypeRegistry().getNativeType(STRING_TYPE),
                                   DeclarationGenerator.this.compiler.getTypeRegistry().getNativeType(ALL_TYPE), true);
            }
            ...
            visitInstanceProperties(prototype, ((ObjectType) instanceType).getOwnPropertyNames(), superClassFields, classTemplateTypeNames);
         ...
         *
         * @param isInNamespace
         *                 if true, emit the properties in a form suitable for namespace members
         *                 (e.g. with "let" and "function" prefixes). If false, emit suitable for properties
         *                 declared in a class or interface.
         */
        private void visitStaticProperties(ObjectType objType, Set<String> propNames, boolean isInNamespace) {
            for (String propName : propNames) {
                ...
                visitProperty(propName, objType, !isInNamespace, false, isInNamespace, Collections.emptyList());
            }
        }

        private void visitInstanceProperties(ObjectType objType, Set<String> skipNames, Set<String> forceProps, List<String> classTemplateTypeNames) {
            for (String propName : getTypePropertyNamesToEmit(objType, false)) {
              if (skipNames.contains(propName)) {
                continue;
              }
                visitProperty(propName, objType, false, forceProps.contains(propName), false, classTemplateTypeNames);
            }
        }
        ...
        private void maybeEmitSymbolIterator(JSType instanceType) {
            ...
            } else if (DeclarationGenerator.this.iterableType != null && instanceType.isSubtype(DeclarationGenerator.this.iterableType)
        ...
        private void aggregateFieldsFromClass(Set<String> fields, ObjectType superType) {
            ...
            if (superType.getConstructor() != null && superType.getConstructor().getPrototype() != null
        ...
        private void visitProperty(String propName, ObjectType objType, boolean isStatic, boolean forcePropDeclaration, boolean isNamespace,
          ...
          if (isStatic && isFunctionPrototypeProp(propName)) {
            ...
            emitProperty(propName, propertyType, isStatic, isProtected, isAbstract, forcePropDeclaration, isNamespace, classTemplateTypeNames);
        }
        ...
        private void emitProperty(String propName, JSType propertyType, boolean isStatic, boolean isProtected, boolean isAbstract, boolean forcePropDeclaration,
                                  boolean isNamespace, List<String> classTemplateTypeNames) {
          ...
          if (isProtected) {
            ...
            if (!propertyType.isFunctionType() || forcePropDeclaration) {
                ...
                if (unionType != null && unionType.getAlternates().stream().anyMatch(JSType::isVoidType) && !isNamespace) {
                    emit("?");
                ...
                List<String> skipTemplateParams = isStatic ? Collections.emptyList() : classTemplateTypeNames;
        ...
        private boolean specialCaseMapEntries(String propName, FunctionType propertyType) {
            ...
            if (ttypes == null || !returnType.getDisplayName().equals("IteratorIterable") || ttypes.size() != 1 || !isTemplateOf(ttypes.get(0), "Array")) {
                return false;
            }
            JSType arrayMembers = ttypes.get(0).toMaybeTemplatizedType().getTemplateTypes().get(0);
            if (!arrayMembers.isUnionType() || arrayMembers.toMaybeUnionType().getAlternates().size() != 2) {
                return false;
            }
        ...
        private boolean isTemplateOf(JSType type, String typeName) {
            return type.isTemplatizedType() && type.toMaybeTemplatizedType().getReferenceName().equals(typeName);
        }
         ...
         */
        private boolean handleSpecialTTEFunctions(JSType type, String propName, boolean isStatic, List<String> classTemplateTypeNames) {
          ...
          if (ftype == null) {
            ...
            if (info != null) {
                boolean isUntypedOverride = info.isOverride() && info.getParameterCount() == 0 && info.getReturnType() == null;
          ...
          if (!horribleHackForPartialModeWithOverrides && !hasTTE) {
            return false;
          }
        ...
        private String getSignatureForInstanceTTEFn(String propName, List<String> classTemplateTypeNames, FunctionType ftype) {
            // If ftype is foo.bar.Promise.prototype.then, extract className as ಠ_ಠ.clutz.foo.bar.Promise.
            String className = Constants.INTERNAL_NAMESPACE + "." + getNamespace(getNamespace(ftype.getDisplayName()));
          ...
          if (!templateTypeNames.hasNext()) {
            ...
            String classTemplatizedType = className.equals("ಠ_ಠ.clutz.goog.Promise") ? " any" : className + " < RESULT >";
            ...
            if (propName.equals("then")) {
                if (className.equals("ಠ_ಠ.clutz.angular.$q.Promise")) {
                    return "then < RESULT > (opt_onFulfilled ? : ( (a : " + templateVarName + " ) => " + classTemplatizedType + " | RESULT | " + className
                                    + "<never>) | null , " + "opt_onRejected ? : ( (a : any ) => any ) | null) : " + classTemplatizedType + " ;";
                } else {
                    return "then < RESULT > (opt_onFulfilled ? : ( (a : " + templateVarName + " ) => PromiseLike < RESULT > | RESULT ) | null , "
                                    + "opt_onRejected ? : ( (a : any ) => any ) | null) : " + classTemplatizedType + " ;";
                }
            }
            if (propName.equals("when")) {
                return "when < RESULT, T > (value: T, successCallback: (promiseValue: T) => " + classTemplatizedType
                                + "|RESULT, errorCallback: null | undefined | " + " ((reason: any) => any), notifyCallback?: (state: any) => any): "
        ...
        private List<String> getTemplateTypeNames(ObjectType objType) {
            ...
            return objType.getTemplateTypeMap()
                          .getTemplateKeys()
                          .stream()
        ...
        private boolean shouldSkipEmittingThisTemplateAndParam(FunctionType ftype) {
            final JSType typeOfThis = ftype.getTypeOfThis();
            if (typeOfThis == null || !typeOfThis.isTemplateType() || !typeOfThis.equals(ftype.getReturnType())) {
                return false;
            }
            Iterator<Node> parameters = ftype.getParameters().iterator();
            while (parameters.hasNext()) {
            ...
            if (type.isNoType() && DeclarationGenerator.this.childListMap.containsKey(innerNamespace)) {
                ...
                for (TypedVar symbol : DeclarationGenerator.this.childListMap.get(innerNamespace)) {
                    if (getNamespace(symbol.getName()).equals(innerNamespace)) {
                        innerProps.put(new NamedTypePair(symbol.getType(), getUnqualifiedName(symbol.getName())), symbol.getNode());
                    }
                }
            } else {
                ...
                for (String propName : getEmittablePropertyNames(type)) {
                    innerProps.put(new NamedTypePair(type.getPropertyType(propName), propName), nodes.get(innerNamespace + "." + propName));
                }
            }
              ...
              if (this.provides.contains(qualifiedName)) {
                ...
                if (pType.isEnumType()) {
                    ...
                    if (registryType != null) {
                        ...
                        emitComment("Intended to visit type alias '" + innerNamespace + "." + propName + " but type not found in Closure type registry.");
                    }
                }
        ...
        public Void emitObjectType(ObjectType type, boolean inExtendsImplementsPosition) {
            ...
            if (!type.getTemplateTypeMap().isEmpty() && !this.typeRegistry.getNativeType(OBJECT_TYPE).equals(type)) {
    ...
    private boolean isOrdinaryFunction(JSType ftype) {
        ...
        boolean ordinaryFunctionAppearingAsClass = ftype.isConstructor() && "Function".equals(ftype.getDisplayName());
    ...
    private void emitSkipTypeAlias(TypedVar symbol) {
        emit("/* skipped emitting type alias " + symbol.getName() + " to avoid collision with existing one in lib.d.ts. */");
    }
}
