...
public class Options {
    ...
    @Option(name = "--externs", usage = "list of files to read externs definitions (as separate args)", metaVar = "EXTERN...", handler =
                    StringArrayOptionHandler.class)
    List<String> externs = new ArrayList<>();

    @Option(name = "--closure_env", usage = "Determines the set of builtin externs to load. Options: BROWSER, CUSTOM. " + "Default: no builtin externs")
    CompilerOptions.Environment closureEnv = null;

    @Option(name = "--depgraphs", usage = "only generate output for files listed as a root in the given depgraphs", metaVar = "file.depgraph...", handler =
                    StringArrayOptionHandler.class)
    List<String> depgraphFiles = new ArrayList<>();

    @Option(name = "--strict_deps", usage = "generates no modules for nonroots (but does generate types), so that nonroots "
                    + "cannot be imported by TypeScript code.")
    boolean strictDeps = false;

    @Option(name = "--depgraphs_filter_sources", usage = "only include sources from the arguments list that appear in the given depgraphs")
    boolean filterSourcesWithDepgraphs = false;

    @Option(name = "--closure_entry_points", usage = "only generate output for the given entry points to the program. Must be"
                    + " goog.provide'd symbols.", metaVar = "ENTRYPOINT...", handler = StringArrayOptionHandler.class)
    List<String> entryPoints = new ArrayList<>();

    @Option(name = "--partialInput", usage = "allow input of incomplete programs. All unknown types will be treated as forward" + " declared.")
    boolean partialInput;

    @Option(name = "--skipEmitRegExp", usage = "Symbols in files that match this RegExp will not be included in the emit. Note that"
                    + "the files would still be part of the internal compilation.")
    String skipEmitRegExp = null;

    @Option(name = "--collidingProvides", usage = "file containing a list of names that we know conflict with namespaces")
    ...
    @Option(name = "--tracer_mode", hidden = true, usage = "Shows the duration of each compiler pass and the impact to " + "the compiled output size. "
                    + "Options: ALL, AST_SIZE, RAW_SIZE, TIMING_ONLY, OFF")
    private final CompilerOptions.TracerMode tracerMode = CompilerOptions.TracerMode.OFF;

    @Option(name = "--browserResolverStrippedPrefixes", usage = "A list of prefixes for absolute ES6 module paths, that would be replaced by '/'", handler =
                    StringArrayOptionHandler.class)
    ...
    public CompilerOptions getCompilerOptions() {
        ...
        if (this.entryPoints.isEmpty()) {
            ...
            ImmutableList<ModuleIdentifier> entryPointIdentifiers = this.entryPoints.stream().map(ModuleIdentifier::forClosure).collect(toImmutableList());
            options.setDependencyOptions(DependencyOptions.pruneLegacyForEntryPoints(entryPointIdentifiers));
        }
        ...
        for (String str : this.browserResolverStrippedPrefixes) {
            builder.put(str, "/");
        }
        options.setBrowserResolverPrefixReplacements(builder.build());

        if (this.closureEnv != null) {
            options.setEnvironment(this.closureEnv);
        }
    ...
    Options(String[] args) throws CmdLineException {
        ...
        if (this.skipEmitRegExp != null) {
            this.skipEmitPattern = Pattern.compile(this.skipEmitRegExp);
        }
        this.depgraph = Depgraph.parseFrom(this.depgraphFiles);
        if (this.filterSourcesWithDepgraphs) {
            ...
            Set<String> merged = Sets.union(this.depgraph.getRoots(), this.depgraph.getNonroots());
            this.arguments.retainAll(merged);
        }

        if (!this.partialInput) {
            // set union command line externs and depgraph.externs.
            Set<String> allExterns = new LinkedHashSet<>();
            allExterns.addAll(this.depgraph.getRootExterns());
            allExterns.addAll(this.depgraph.getNonrootExterns());
            allExterns.addAll(this.externs);
            this.externs = new ArrayList<>(allExterns);
        }
        ...
        this.arguments.removeAll(this.externs);
        if (!this.strictDeps) {
            this.depgraph = this.depgraph.withNonrootsAsRoots();
        }
        if (this.arguments.isEmpty() && this.externs.isEmpty()) {
            throw new CmdLineException(parser, "No files or externs were given");
        }

        if (this.collidingProvidesFile != null) {
            try {
                this.collidingProvides.addAll(Files.readLines(new File(this.collidingProvidesFile), UTF_8));
            } catch (IOException e) {
                throw new RuntimeException("Error reading aliased names file " + this.collidingProvidesFile, e);
            }
        }
    }
