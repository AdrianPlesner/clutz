    ...
    protected Map<String, String> build(String localModuleId, Node moduleBody, Set<String> googProvides) {
        ...
        for (Node statement : moduleBody.children()) {
            if (isImportAssignment(statement)) {
                ...
                String importedModuleId = statement.getFirstChild().getChildAtIndex(1).getChildAtIndex(1).getString();
        ...
        for (Node statement : moduleBody.children()) {
            if (isWholeModuleExportAssignment(statement)) {
                ...
                if (localVariableToImportedSymbolNameMap.containsKey(localVariableName)) {
                    aliasMap.put(buildWholeModuleExportSymbolName(localModuleId), localVariableToImportedSymbolNameMap.get(localVariableName));
                }
                ...
                if (localVariableToImportedSymbolNameMap.containsKey(localVariableName)) {
                    aliasMap.put(buildNamedExportSymbolName(localModuleId, exportName), localVariableToImportedSymbolNameMap.get(localVariableName));
                }
                ...
                String exportName = getNamedExportName(statement);
                String localNamespaceName = localVariableToImportedSymbolNameMap.containsKey(localVariableName) ? localVariableToImportedSymbolNameMap.get(
                                localVariableName) : localVariableName;
                aliasMap.put(buildNamedExportSymbolName(localModuleId, exportName), localNamespaceName + "." + localPropName);
            }
        }
